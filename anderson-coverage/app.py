...
@app.route("/")
def index():
    # Show a "Back" link only when served under a parent app (SCRIPT_NAME set,
    # e.g. mounted at /anderson-coverage). Standalone has nowhere to go back to.
    parent_url = "/anderson/" if request.script_root else None
...
<style>
  * { box-sizing: border-box; }
  ...
  .back-btn { display: inline-flex; align-items: center; gap: 7px; flex-shrink: 0;
        background: #fff; color: #005FA0; border: 1px solid #005FA0; padding: 8px 15px;
  ...
  @media print {
    .searchbar, .toolbar, .back-btn { display:none; }
...
<div class="report-header">
  <div class="wrap hdr-inner">
    {% if parent_url %}<a class="back-btn" href="{{ parent_url }}">&#8592; Back</a>{% endif %}
